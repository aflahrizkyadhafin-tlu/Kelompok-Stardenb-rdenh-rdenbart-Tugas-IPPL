...
class LogPengirimanController extends GetxController {
  Future<void> catatLog(String idPengiriman) async {
    try {
      Pengiriman? checkPengiriman = await PengirimanController.getPengirimanById(idPengiriman);

      if (checkPengiriman != null){
        Map jsonData = LogPengiriman(idLog: "", statusPengiriman: checkPengiriman.statusPengiriman, waktuUpdate: DateTime.now(), idPengiriman: idPengiriman).toJson();
